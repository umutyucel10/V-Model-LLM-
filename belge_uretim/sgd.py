...
def run_generation_logic(
    ...
    try:
        ...
        # büyük bir hedef (>15-20) istemek hem generate_sgd_batch'in token
        ...
        SUB_BATCH_SIZE = 15
        max_sub_batch_rounds = max((max_sgds // SUB_BATCH_SIZE) + 3, 4)
        for _round in range(max_sub_batch_rounds):
            if len(sgd_list) >= max_sgds:
                break
            before = len(sgd_list)
            sub_count = min(SUB_BATCH_SIZE, max_sgds - len(sgd_list))
            for sgd in generate_sgd_batch(top_text, sub_count, avoid_list=existing_texts):
                if len(sgd_list) >= max_sgds:
                    break
                if _uygun(sgd):
