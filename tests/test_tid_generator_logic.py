# -*- coding: utf-8 -*-
"""generate_tid_batch / run_generation_logic buyuk hedef sayilarinda madde
...
import tid_generator_logic as generator
...
class GenerateTidBatchTokenBudgetTests(unittest.TestCase):
    """generate_tid_batch, count buyudukce max_tokens'i eski 700 tavanina
    ...
    def test_small_count_uses_scaled_token_budget(self):
        ...
        with patch.object(generator, "call_gemma3_api", model):
            generator.generate_tid_batch("kaynak metin", 8)
    ...
    def test_large_count_is_not_capped_at_old_700_ceiling(self):
        model = _DiverseBatchModel()
        with patch.object(generator, "call_gemma3_api", model):
            generator.generate_tid_batch("kaynak metin", 25)
        self.assertEqual(len(model.calls), 1)
    ...
    def test_returns_requested_item_count_when_model_cooperates(self):
        model = _DiverseBatchModel()
        with patch.object(generator, "call_gemma3_api", model):
            items = generator.generate_tid_batch("kaynak metin", 25)
        self.assertEqual(len(items), 25)
        self.assertEqual(len(set(items)), 25)  # hepsi birbirinden farkli


class RunGenerationLogicSubBatchTests(unittest.TestCase):
    """run_generation_logic, buyuk max_tids hedeflerinde tek dev cagri yerine
    ...
    def test_reaches_large_target_via_sub_batches(self):
        ...
        self.assertEqual(len(result["tid_list"]), 25)
    ...
    def test_small_target_still_works_in_single_round(self):
        ...
        self.assertTrue(result["result"])
        self.assertEqual(len(result["tid_list"]), 8)
